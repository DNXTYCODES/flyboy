...
const ProductItem = ({id,image,name,price}) => {
  ...
  <Link onClick={()=>scrollTo(0,0)} className='text-goldenrod cursor-pointer' to={`/product/${id}`}>
      <div className=' overflow-hidden'>
        <img className=' rounded-xl hover:scale-110 transition ease-in-out' src={image[0]} alt="" />
      </div>
      <p className='pt-3 pb-1 text-sm bg-golden-brown bg-clip-text text-transparent bg-to-b'>{name}</p>
